...
class Xception(nn.Module):
    ...
    def __init__(self, xception_config):
        ...
        # print("conv4 in backbone:", hasattr(self, 'conv4'))
    ...
    def classifier(self, features, id_feat=None, get_embeddings=False):
        # for iid
        if self.mode == 'adjust_channel':
            x = features
        else:
        ...
        if len(x.shape) == 4:
            ...
            x = x.view(x.size(0), -1)
        self.last_emb = x  # Save task-agnostic embeddings
        ...
        # Adjust logic for returning embeddings
        if get_embeddings:
            return out, x  # Return logits and embeddings
        return out
