...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def __init__(self, config=None, mode='train'):
        ...
        Args:
            config (dict): A dictionary conta
            ining configuration parameters.
    ...
    def collect_img_and_label_for_one_dataset(self, dataset_name: str):
        ...
        for label in dataset_info[dataset_name]:
            ...
            # Choose the compression type if applicable
            ...
            for video_name, video_info in sub_dataset_info.items():
                ...
                # Remove frame paths that end with four digits followed by .png
                original_length = len(frame_paths)
                frame_paths = self.filter_frame_paths(frame_paths)
                removed = original_length - len(frame_paths)
                if removed > 0:
                    print(f"Removed {removed} frames ending with four digits followed by .png in video {unique_video_name}.")
    
                # Update video_info['frames']
                video_info['frames'] = frame_paths
    
                # Check if any frames remain after filtering
                if len(frame_paths) == 0:
                    print(f"No valid frames left in video {unique_video_name} after filtering. Skipping this video.")
                    continue
    ...
    def collate_fn(batch):
        ...
        # Process landmarks and masks as before
        ...
        data_dict['image_path'] = image_paths  # Add this line
