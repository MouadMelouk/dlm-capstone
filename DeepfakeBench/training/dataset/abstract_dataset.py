...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def __init__(self, config=None, mode='train'):
        ...
        lmdb_path = f"/datasets/rgb/FaceForensics++/lmdb/{'flows' if use_flows else 'frames'}/pairstride{pairstride}_srcstride{srcstride}/"

        self.lmdb_path = lmdb_path

        if not os.path.exists(lmdb_path):
            # Create LMDB
            print(f"LMDB does not exist at {lmdb_path}. Creating LMDB...")
            ...
            # Estimate total size, assuming average image size is 300KB
            map_size = total_images * 800 * 1024

            env = lmdb.open(lmdb_path, map_size=map_size)
                        
            with env.begin(write=True) as txn:
                for idx, image_path in enumerate(tqdm(self.image_list, desc="Creating LMDB")):
                    # Adjust image_path to absolute path
                    if not os.path.isabs(image_path):
                        image_path_abs = os.path.join(self.config.get('rgb_dir', ''), image_path)
                    else:
                        image_path_abs = image_path
            
                    assert os.path.exists(image_path_abs), f"Image path {image_path_abs} does not exist"
                    ...
                    # Store in LMDB
                    key = str(idx).zfill(8).encode('ascii')
                    txn.put(key, img_bin)
            ...
            print(f"LMDB exists at {lmdb_path}. Using existing LMDB.")

        # Open LMDB for reading
        self.env = lmdb.open(lmdb_path, readonly=True, lock=False, readahead=False, meminit=False)

        # Set self.lmdb to True
        self.lmdb = True
        ...
        for label in dataset_info[dataset_name]:
            ...
            for video_name, video_info in sub_dataset_info.items():
                ...
                if use_flows:
                    ...
                    # Replace `/frames/` with `/flows/pairstride{pairstride}_srcstride{srcstride}/`
                    flow_dir = "/datasets/rgb/" + directory.replace(
                        "/frames/",
                        f"/flows/pairstride{pairstride}_srcstride{srcstride}/"
                    )
    ...
    def load_rgb(self, index):
        """
        Load an RGB image from LMDB using index.

        Args:
            index: An integer index of the image.
        ...
        """
        key = str(index).zfill(8).encode('ascii')

        with self.env.begin(write=False) as txn:
            img_bin = txn.get(key)

        if img_bin is None:
            raise ValueError(f'Image not found in LMDB with key {key}')

        img = cv2.imdecode(np.frombuffer(img_bin, np.uint8), cv2.IMREAD_COLOR)

        if img is None:
            raise ValueError(f'Failed to decode image from key {key}')
    ...
    def __getitem__(self, index, no_norm=False):
        ...
        # Load the image from LMDB using index
        try:
            image = self.load_rgb(index)
        except Exception as e:
