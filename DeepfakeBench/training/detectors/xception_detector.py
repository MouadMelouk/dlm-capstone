...
class XceptionDetector(AbstractDetector):
    ...
    def generate_gradcam(self, input_image, target_class=None, image_path="./gradcam_outputs"):    
        detector_type = "Xception"  # Updated to reflect detector type
        datasets_base_path = "/datasets/rgb/"
        ...
        # Modify save path to replace '/frames/' with '/frame_gradcam/Xception/'
        save_path = os.path.join(datasets_base_path, os.path.dirname(image_path).replace("/flows/", "/flow_gradcam/flows_pairstride1_srcstrice16/Xception/"))
        ...
        print(f"GradCAM output saved to {full_save_path}.")
        
        cv2.imwrite(full_save_path, heatmap)
