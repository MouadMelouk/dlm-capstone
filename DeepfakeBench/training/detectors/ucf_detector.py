...
class UCFDetector(AbstractDetector):
    ...
    def generate_gradcam(self, input_image, target_class=None, image_path="./gradcam_outputs"):
        ...
        #print(f"Saving Grad-CAM heatmap to: {save_path}")
    ...
    def generate_embedding(self, data, save_path):
        ...
        with torch.no_grad():
            ...
            # Forward pass with get_embeddings=True
            pred_dict = self.forward(data_dict, inference=True, get_embeddings=True)
            # Get the embeddings
            embedding = pred_dict['embeddings']
