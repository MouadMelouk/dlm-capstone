...
def get_test_metrics(y_pred, y_true, img_names):
    def save_csv(filename, rows):
        with open(filename, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['frame_path', 'real_label', 'prediction'])
    ...
    # Save predictions and labels for each frame
    rows = [(img_name, label, pred) for img_name, label, pred in zip(img_names, y_true, y_pred)]
    save_csv('/scratch/mmm9912/Capstone/DeepfakeBench/statistics/labels_and_predictions_of_frames_2_.csv', rows)
