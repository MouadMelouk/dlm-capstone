...
from DeepFake_dataloader import NonSequentialFramePairDataset
...
def custom_collate_fn(batch):
    images1, images2, paths = zip(*batch)
    images1 = torch.stack(images1, 0)
    images2 = torch.stack(images2, 0)
    img1_paths, img2_paths = zip(*paths)
    return images1, images2, (img1_paths, img2_paths)
    

@torch.no_grad()
def evaluate_dataset(model, folder_paths, pair_stride, src_stride, sigma=0.05):
    dataset = NonSequentialFramePairDataset(folder_paths, pair_stride, src_stride)
    ...
    description = f"Processing pairs (pair_stride={pair_stride}, src_stride={src_stride})"

    for batch_idx, (images1, images2, (img1_paths, img2_paths)) in enumerate(tqdm(data_loader, desc=description)):
        ...
        flow_pre, _ = model(images1, images2)
        flows = padder.unpad(flow_pre).cpu()
        
        for i in range(flows.size(0)):
            flow = flows[i].permute(1, 2, 0).cpu().numpy()
        
            img1_path = img1_paths[i]
            img2_path = img2_paths[i]
            modified_folder_path = modify_folder_path(img1_path, pair_stride, src_stride)
            output_filename = get_this_flow_s_path(img1_path, img2_path, modified_folder_path)
        
            flow_img = flow_viz.flow_to_image(flow)
            cv2.imwrite(output_filename, flow_img[:, :, ::-1])  # Convert RGB to BGR for OpenCV
            #print(f"Saved flow visualization for pair {batch_idx * 32 + i} to {output_filename}")


def modify_folder_path(folder_path: str, pair_stride: int, src_stride: int) -> str:
    folder_path = re.sub(
        r"/frames/",
        f"/flows/pairstride{pair_stride}_srcstride{src_stride}/",
        folder_path
    )
    folder_path = re.sub(r"/\d{3,4}\.png$", "", folder_path)
    
    return folder_path


def get_this_flow_s_path(img1_path: str, img2_path: str, folder_path: str) -> str:
    # Extract the 3 or 4-digit numbers from the image paths
    img1_number = re.search(r"/(\d{3,4})\.png$", img1_path).group(1)
    img2_number = re.search(r"/(\d{3,4})\.png$", img2_path).group(1)
    
    # Ensure the folder exists
    os.makedirs(folder_path, exist_ok=True)
    
    # Generate the filename based on the extracted numbers
    this_flow_s_path = f"{img1_number}_{img2_number}.png"
    return os.path.join(folder_path, this_flow_s_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='./checkpoints/sintel.pth', help='path to the model checkpoint')
    parser.add_argument('--ps', type=int, required=True, help='stride between frames in a pair')
    parser.add_argument('--ss', type=int, required=True, help='stride between source images from consecutive pairs')
    ...
    # Hardcoded list of folder paths of FF++ c23
    # These specific subfolders were used for training DeepfakeBench, and none other. 
    folder_paths = [
        '/datasets/rgb/FaceForensics++/manipulated_sequences/Deepfakes/c23/',
        '/datasets/rgb/FaceForensics++/manipulated_sequences/Face2Face/c23/',
        '/datasets/rgb/FaceForensics++/manipulated_sequences/FaceSwap/c23/',
        '/datasets/rgb/FaceForensics++/manipulated_sequences/NeuralTextures/c23/',
        '/datasets/rgb/FaceForensics++/original_sequences/youtube/c23/',
        '/datasets/rgb/FaceForensics++/original_sequences/actors/c23/'
    ]

    # Collect all subfolder paths inside each {folder_path}/frames/
    all_subfolder_paths = []
    for folder_path in folder_paths:
        subfolders = get_frame_subfolders(folder_path)
        all_subfolder_paths.extend(subfolders)

    # Run evaluation on all subfolder paths
    evaluate_dataset(model.module, all_subfolder_paths, pair_stride, src_stride)
